...
def optimize_model():
    ...
    non_final_mask = torch.tensor(tuple(map(lambda s: s is not None, batch.next_sate)), device=device, dtype=torch.bool)
...
while n_steps < total_steps:
    ...
    for t in count():
        ...
        observation = torch.tensor(observation, dtype=torch.float32, device=device).view(-1)
