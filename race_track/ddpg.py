...
class DDPG_agent:
    def __init__(self, env, continueTraining=False):
        ...
        self.actor_scheduler = optim.lr_scheduler.StepLR(
            self.actor_optimizer, step_size=10000, gamma=0.95
        )
        self.critic_scheduler = optim.lr_scheduler.StepLR(
            self.critic_optimizer, step_size=10000, gamma=0.95
        )
    ...
    def update(self, global_step):
        ...
        next_obs = (
            torch.FloatTensor(next_obs).reshape(self.batch_size, -1).to(self.device)
        )
